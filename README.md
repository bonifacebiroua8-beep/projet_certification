...
- 📱 **5 écrans** : Login, Register, Home (4 onglets), Profil, Paramètres
...
- ⚠️ **Gestion d'erreurs différenciée** (réseau / timeout / serveur)
...
│ ├── storage/ # SecureStorage (tokens), LocalDb (SQLite)
│ └── utils/ # JwtUtils
...
flutter analyze (0 warnings exigé)

flutter test (tous les tests unit + widgets)
...
```powershell
git add .
git commit -m "docs: README pro + CHANGELOG 3 versions"
git push
